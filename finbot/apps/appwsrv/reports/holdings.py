...
from finbot.apps.appwsrv import repository, timeseries
...
def generate(session, history_entry: UserAccountHistoryEntry):
    user_account_valuation = repository.find_user_account_valuation(session, history_entry.id)
    linked_accounts_valuation = repository.find_linked_accounts_valuation(session, history_entry.id)
    ...
    user_account_historical_valuation = repository.find_user_account_historical_valuation(
        session=session,
        user_account_id=history_entry.user_account_id,
        from_time=from_time,
        to_time=to_time)
    historical_valuation_by_account = repository.find_linked_accounts_historical_valuation(
        session=session,
        user_account_id=history_entry.user_account_id,
        from_time=from_time,
        to_time=to_time)
