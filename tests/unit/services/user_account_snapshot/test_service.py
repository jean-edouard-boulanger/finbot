...
@pytest.fixture(scope="function")
def snapshot_with_invalid_asset_missing_ccy() -> list[service.LinkedAccountSnapshotResult]:
    return [
        service.LinkedAccountSnapshotResult(
            request=service.LinkedAccountSnapshotRequest(
                linked_account_id=1,
...
@pytest.fixture(scope="function")
def snapshot_with_global_error() -> list[service.LinkedAccountSnapshotResult]:
    return [
        service.LinkedAccountSnapshotResult(
            request=service.LinkedAccountSnapshotRequest(
                linked_account_id=1,
            ...
            snapshot_data=core_schema.ApplicationErrorData(user_message="Fatal error while taking snapshot"),
        ),
    ]


@pytest.fixture(scope="function")
def snapshot_with_invalid_asset_duplicate_values() -> list[service.LinkedAccountSnapshotResult]:
    return [
        service.LinkedAccountSnapshotResult(
            request=service.LinkedAccountSnapshotRequest(
                linked_account_id=1,
...
class TestXccyCollector:
    ...
    def test_raises_when_value_in_item_ccy_is_specified_but_currency_is_missing(
        self,
        snapshot_with_invalid_asset_missing_ccy: list[service.LinkedAccountSnapshotResult],
    ):
        collector = service.XccyCollector(target_ccy=TEST_USER_ACCOUNT_CCY)
        with pytest.raises(InconsistentSnapshotData):
            service.visit_snapshot_tree(snapshot_with_invalid_asset_missing_ccy, collector)

    def test_raises_when_both_value_in_account_ccy_and_item_ccy_are_provided(
        self,
        snapshot_with_invalid_asset_duplicate_values: list[service.LinkedAccountSnapshotResult],
    ):
        collector = service.XccyCollector(target_ccy=TEST_USER_ACCOUNT_CCY)
        with pytest.raises(InconsistentSnapshotData):
            service.visit_snapshot_tree(snapshot_with_invalid_asset_duplicate_values, collector)
