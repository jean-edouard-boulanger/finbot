...
export const SpendingBreakdownPanel: React.FC<SpendingBreakdownPanelProps> = (
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (fromDate) {
          params.set(
            "from_time",
            DateTime.fromISO(fromDate).startOf("day").toISO()!,
          );
        }
        if (toDate) {
          params.set("to_time", DateTime.fromISO(toDate).endOf("day").toISO()!);
        }
    ...
    fetchData();
  }, [accessToken, userAccountId, fromDate, toDate]);
    ...
    <Card className="border-border/50 flex flex-col h-full">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <DateRangeFilter
          fromDate={fromDate}
          toDate={toDate}
          onFromDateChange={(v) => setRange((r) => ({ ...r, fromDate: v }))}
          onToDateChange={(v) => setRange((r) => ({ ...r, toDate: v }))}
          compact
          allowAllTime
        />
      </CardHeader>
